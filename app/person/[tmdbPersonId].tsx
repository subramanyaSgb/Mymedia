...
import { Chip, haptic, Icon, SectionHeader, Text, useColors, useThemedStyles } from '@/components/ui';
import { space, radius, type Palette } from '@/constants/theme';
...
export default function PersonScreen() {
  ...
  if (!person) {
    return (
      <View style={styles.center}>
        ...
        <Text color={c.textMuted}>Person not found</Text>
      </View>
    );
  }
  ...
  return (
    <ScrollView style={styles.screen} contentContainerStyle={styles.content} bounces={false}>
      <Stack.Screen options={{ title: person.name, headerTransparent: true, headerTintColor: c.text }} />
      ...
      <View style={styles.heroSection}>
        {person.profile_path ? (
          ...
          <View style={[styles.profileImage, styles.profileFallback]}>
            <Icon name="person" size={64} color={c.textFaint} />
          </View>
        )}
        <LinearGradient colors={['transparent', c.bg]} style={styles.heroGradient} />
      ...
      <View style={styles.body}>
        ...
        {filtered.length === 0 ? (
          <View style={styles.empty}>
            <Text color={c.textMuted}>No filmography available</Text>
          </View>
        ) : (
          <>
                    ...
                    <View>
                      {f.poster ? (
                        ...
                        <View style={[styles.poster, styles.posterFallback]}>
                          <Icon name="image" size={32} color={c.textFaint} />
                        </View>
                      )}
                      <Pressable
                        accessibilityRole="button"
                        ...
                        <Icon name={added ? 'checkmark' : 'add'} size={18} color={c.onAccent} />
                      </Pressable>
                    </View>
...
const makeStyles = (c: Palette) => StyleSheet.create({
  ...
  profileFallback: { backgroundColor: c.surface, alignItems: 'center', justifyContent: 'center' },
  ...
  posterFallback: { backgroundColor: c.surface, alignItems: 'center', justifyContent: 'center' },
