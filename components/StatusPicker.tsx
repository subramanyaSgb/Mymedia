  ...
  return (
    <Modal visible={visible} transparent animationType="slide" onRequestClose={onClose}>
      ...
      <View style={styles.sheet}>
        ...
        {subtitle ? (
          <Text variant="micro" color={c.textMuted} style={styles.subtitle}>
          ...
          return (
            <Pressable
              ...
              <View style={[styles.iconWrap, active && styles.iconWrapActive]}>
                <Icon name={STATUS_ICON[s.key]} size={18} color={active ? c.onAccent : c.accent} />
              </View>
              <Text variant="bodyStrong" style={{ flex: 1 }}>
                {s.label}
              </Text>
              {active ? <Icon name="checkmark" size={18} color={c.accent} /> : null}
            </Pressable>
          );
        })}
        <Pressable
          ...
          <Text variant="bodyStrong" color={c.textMuted}>
            Cancel
          </Text>
        </Pressable>
      </View>
...
const makeStyles = (c: Palette) => StyleSheet.create({
  ...
  rowActive: { backgroundColor: c.accentDim },
  iconWrap: {
    width: 36,
    height: 36,
    borderRadius: 18,
